...
无需等全部生成完，中断后从最后一个完整batch继续

策略：只使用SFT模型推理错误的样本
- chosen = 原始正确CoT
- rejected = SFT模型的错误输出（自动清理prompt残留）
...
def build_prompt(question, instruction):
    """
    构建与SFT训练时一致的prompt
    如果instruction为空，则省略system字段（避免空system message）
    """
    if instruction and instruction.strip():
        ...
        # 无system instruction时，使用Qwen默认对话格式
        return (
            f"|<|im_start|>user\n{question.strip()}\n<|<|im_end|>\n"
            f"|<|im_start|>assistant\n"
        )
...
def sft_generate_batch(model, tokenizer, batch_items):
    """
    批量SFT推理一个batch
    返回：生成的文本列表（与batch_items一一对应，已清理）
    """
    prompts = [
    ...
    with torch.no_grad():
        outputs = model.generate(
            **inputs,
            ...
            do_sample=False,           # greedy decoding，可复现
            temperature=None,          # 配合do_sample=False
...
def process_batch_and_save(batch_items, sft_outputs, dpo_data, processed_ids,
    ...
    # ===== 立即保存（原子写入） =====
    checkpoint = {
        "processed_ids": sorted(list(processed_ids)),  # 排序便于查看
        "dpo_data": dpo_data,
        "last_batch_stats": {
