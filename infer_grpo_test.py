"""
使用 GRPO 训练的最佳模型（qwen_grpo_best LoRA 适配器）对 test.json 进行推理
参考 other_methods/infer_cot.py 的实现方法
"""
...
# ==================== 配置参数 ====================
BASE_MODEL_PATH = "./qwen_sft_full/"              # 基座模型路径
LORA_PATH = "./qwen_grpo_best"                     # GRPO 训练的 LoRA 适配器
TEST_JSON_PATH = "test.json"                       # 测试集路径
OUTPUT_CSV_PATH = "test_predictions_grpo.csv"      # 输出 CSV 文件路径
SAVE_INTERVAL = 50                                  # 每推理 N 条就写入一次文件
MAX_NEW_TOKENS = 256                                # 最大生成 token 数
# ==================================================

# 与 GRPO 训练一致的 CoT 系统提示
SYSTEM_MESSAGE = (
...
def load_model_and_tokenizer(base_path, lora_path):
    """加载基座模型和 GRPO LoRA 适配器"""
    print("正在加载 Tokenizer...")
    tokenizer = AutoTokenizer.from_pretrained(
        base_path, use_fast=False, trust_remote_code=True
    )
    ...
    print(f"正在加载 GRPO LoRA 适配器: {lora_path}")
...
def build_prompt(tokenizer, question):
    """构造与 GRPO 训练一致的 chat template prompt"""
    messages = [
...
def main():
    ...
    for idx, item in enumerate(test_data):
        sample_id = str(item.get("id", item.get("idx", idx)))
    ...
    with tqdm(total=total_pending, desc="推理中") as pbar:
        for sample_id, item in pending_items:
            ...
            # 使用 chat_template 构造 prompt（与训练一致）
